...
if test "$PHP_MEMCACHE" != "no"; then
  ...
  if test "$PHP_MEMCACHE_SESSION" != "no"; then
    ...
    PHP_NEW_EXTENSION(memcache, memcache.c memcache_pool.c memcache_session.c memcache_standard_hash.c memcache_consistent_hash.c, $ext_shared,,-I$session_inc_path)
    ...
    AC_DEFINE(HAVE_MEMCACHE,1,[Whether you want memcache support])
    PHP_NEW_EXTENSION(memcache, memcache.c memcache_pool.c memcache_standard_hash.c memcache_consistent_hash.c, $ext_shared)
  fi
